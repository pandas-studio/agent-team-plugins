...
# ask-codex.sh writes its `=== RESPONSE ===` / `=== END (rc=N) ===` markers
# to its OWN per-call log file (under the dev-trio plugin's workspace log dir,
# e.g. $PWD/.dev-trio/log/<team>/codex-<ts>.log), not to stdout. Capture
# stderr separately so we can scrape the `(log: <path>, rc=<n>)` line
# ask-codex.sh prints there, then awk the response section out of that
# authoritative file. The earlier shape (tee stdout into CODEX_OUT, awk the
# markers from there) silently produced an empty audit body because the
# markers never appear on stdout.
...
RESPONSE_BODY=""
if [ -n "$DEV_CODEX_LOG" ] && [ -f "$DEV_CODEX_LOG" ]; then
