...
for rel in bin/ralph-solo.sh bin/ralph-trio.sh bin/ralph-debate.sh bin/ralph-meta.sh \
           ...
           claude-skills/bootstrap/SKILL.md claude-skills/install-pm/SKILL.md claude-skills/install-stop-hook/SKILL.md \
...
STUB
  ...
  # under $CWD with the stubs on PATH and planner/coder stubbed (no real model).
  run_trio_case() {
    local mode="$1" cwd="$2"
    mkdir -p "$cwd"
    git -C "$cwd" init -q
