...
#        - ask-codex.sh / ask-gemini.sh from dev-trio plugin (needed for trio/meta)
...
for rel in bin/ralph-solo.sh bin/ralph-trio.sh bin/ralph-debate.sh bin/ralph-meta.sh \
           ...
           lib/common.sh lib/manifest.sh lib/roles/planner.md lib/roles/worker.md \
...
if command -v ask-gemini.sh >/dev/null 2>&1; then
  ok "ask-gemini.sh — $(command -v ask-gemini.sh) (dev-trio plugin; needed for ralph-trio NEED RESEARCH branch)"
else
  warn "ask-gemini.sh — missing (install dev-trio plugin; or pass --no-research to ralph-trio.sh)"
fi
