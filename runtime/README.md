...
- 역할과 테스트 명령은 비대화형으로 실행합니다. stdin은 `/dev/null`이며(0.1.3은 CLI의 stdin을
  ...
  전체를 종료합니다. SIGTERM·SIGHUP으로 CLI가 종료될 때도 먼저 그룹을 종료한 뒤 CLI는 그 signal로
  종료합니다. 이후 `status`는 `incomplete`(종료 코드 6)를 보고하고 `resume`으로 이어갈 수 있습니다.
  입력을 묻거나 stdin을 읽는 명령은 EOF를 받으므로 wrapper 스크립트에서
  파일로 입력을 넘기세요.
