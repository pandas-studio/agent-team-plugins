---
description: Install (or upgrade) the debate-conductor PM orchestration policy into the workspace's CLAUDE.md. Idempotent — appends a marker-guarded block; re-running upgrades in place. Run once per workspace after installing the plugin. The policy tells Claude when to dispatch /debate-conductor:run vs handle the request directly.
disable-model-invocation: true
allowed-tools: Read Edit Write
---

# Install the debate-conductor PM policy into workspace `CLAUDE.md`
...
Claude needs to remember per turn whether a request warrants dispatching a Generator-vs-Critic debate vs. answering directly. That decision is per-turn, so the routing rules live in `CLAUDE.md` (loaded every turn). The plugin ships the source of truth at `lib/pm.md`; this skill installs/upgrades it in your workspace without manual copy-paste.

The block is guarded by:

```
<!-- BEGIN debate-conductor PM policy -->
...
<!-- END debate-conductor PM policy -->
```
...
If the Read fails (file not found / env var empty), surface this error to the user verbatim and stop: `debate-conductor plugin install appears broken — lib/pm.md missing under $CLAUDE_PLUGIN_ROOT; reinstall via /plugin install debate-conductor@pandas-studio.`
...
- **Exists** → check for the marker pair `<!-- BEGIN debate-conductor PM policy -->` / `<!-- END debate-conductor PM policy -->`:
...
Do **not** wrap the contents in additional markdown headings — `pm.md` already begins with its own `# CLAUDE.md — debate-conductor orchestration policy` heading. Do **not** rewrite the contents — install verbatim so future upgrades (re-runs of this skill against an updated plugin) replace cleanly.

### 4 · Confirm to the user

After write, tell the user:

> Installed debate-conductor PM policy into `$PWD/CLAUDE.md` (block: `<!-- BEGIN debate-conductor PM policy -->` … `<!-- END debate-conductor PM policy -->`). Start a fresh Claude Code session in this workspace to pick it up — `CLAUDE.md` is loaded at session start.
>
> To upgrade after a plugin update, re-run `/debate-conductor:install-pm`. The block is replaced in place; the rest of your `CLAUDE.md` is untouched.
