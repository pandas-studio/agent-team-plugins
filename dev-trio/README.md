...
- inject a bounded `<workspace_snapshot>` into the prompt for default working tree reviews or explicit git revision ranges (`A..B` or `A...B`), precomputing status, diffs, and untracked files within `DEV_TRIO_SNAPSHOT_MAX_BYTES` (default 64 KiB) and `DEV_TRIO_GIT_TIMEOUT` (default 10.0s per Git call). Custom workspace-aware models that take prompts on argv are also capped by `REGISTRY_ARGV_MAX_BYTES`. When working-tree status contains a sensitive path (`.env*`, `*.pem`, SSH keys, `secrets.json`, `*.tfvars*`, etc.) or a tracked deletion, the snapshot retains status but omits all diff and untracked contents; a range snapshot omits its diff content when a sensitive path changed. This covers moves from tracked files into ignored sensitive files. Moves or copies from ignored or untracked sensitive files into ordinary paths cannot be detected from Git status. The omission notice asks the reviewer to inspect directly. The built-in agy model receives the complete prompt on stdin; set `DEV_TRIO_SNAPSHOT_MAX_BYTES=0` to omit the snapshot entirely;
- record whether the snapshot went in. The manifest keeps `kind=workspace-snapshot` for an injected snapshot and adds `kind=workspace-snapshot-skipped` with the reason otherwise; run.json gets `workspace-snapshot-status` as `ok:<working-tree|range>` or `skipped:<reason>`. The reasons are `disabled` (the opt-out, checked before any snapshot probe), `focus` (a free-form focus or more than one range), `root` (the repository root could not be verified), `timeout` (a Git call hit `DEV_TRIO_GIT_TIMEOUT`), `git-failed` (a range endpoint probe failed), `budget`, `oversize`, `empty`, and `helper-<unsupported|not-worktree|git-failed|parse-failed|failed>`. A timed-out or failed Git call drops the whole snapshot, never a partial one. Every reason except `disabled` and `focus` also prints one `[ask-reviewer] workspace snapshot skipped: <reason>` line on stderr, without paths or content. A range snapshot adds its own inspection rule: it covers only `git diff A..B`, `git log` / `git show` cover the commits in it, and the working-tree `git status` step applies only when the focus asks about the working tree;
...
**Prompts on stdin (agy, claude, codex).** The built-in `agy`, `claude`, `claude-write`, `codex` and `codex-no-memories` models take the prompt on stdin (`"prompt_via": "stdin"`): `agy --input-format text --output-format text`, `claude -p` and `codex exec … -` get no prompt argument. This keeps research context and workspace snapshots out of process arguments and avoids the Linux single-argument limit. Agy documents stdin input in its [headless flag reference](https://antigravity.google/docs/cli/headless/). See the [marketplace README](../README.md#shared-model-configuration).
