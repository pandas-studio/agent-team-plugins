...
- inject a bounded `<workspace_snapshot>` into the prompt for default working tree reviews or explicit git revision ranges (`A..B`), precomputing status, diffs, and untracked files within `DEV_TRIO_SNAPSHOT_MAX_BYTES` (default 64 KiB, capped by `REGISTRY_ARGV_MAX_BYTES`) and `DEV_TRIO_GIT_TIMEOUT` (default 10.0s). When status contains a sensitive path (`.env*`, `*.pem`, SSH keys, `secrets.json`, `*.tfvars*`, etc.) or a tracked deletion, the snapshot retains status but omits all diff and untracked contents; a range snapshot omits its diff content in the same case. This covers moves from tracked files into ignored sensitive files. Moves or copies from ignored or untracked sensitive files into ordinary paths cannot be detected from Git status. The omission notice asks the reviewer to inspect directly. Because complete snapshots place workspace diffs and untracked contents into agy's command-line argument (visible to local users via process listings on multi-user systems without `hidepid`), you can opt out entirely by setting `DEV_TRIO_SNAPSHOT_MAX_BYTES=0`;
