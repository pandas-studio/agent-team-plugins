...
- record whether the snapshot went in. The manifest keeps `kind=workspace-snapshot` for an injected snapshot and adds `kind=workspace-snapshot-skipped` with the reason otherwise; run.json gets `workspace-snapshot-status` as `ok:<working-tree|range>` or `skipped:<reason>`. The reasons are `disabled` (the opt-out, checked before any snapshot probe), `focus` (a free-form focus or more than one range), `root` (the repository root could not be verified), `timeout` (a Git call hit `DEV_TRIO_GIT_TIMEOUT`; the whole snapshot is dropped, never a partial one), `budget`, `oversize`, `empty`, and `helper-<unsupported|not-worktree|git-failed|parse-failed|failed>`. Every reason except `disabled` and `focus` also prints one `[ask-reviewer] workspace snapshot skipped: <reason>` line on stderr, without paths or content. A range snapshot adds its own inspection rule: it covers only `git diff A..B`, `git log` / `git show` cover the commits in it, and the working-tree `git status` step applies only when the focus asks about the working tree;
