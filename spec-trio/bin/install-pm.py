...
def install(workspace: Path, host: str) -> Path:
    ...
    begin = "<!-- BEGIN spec-trio PM policy -->"
    end = "<!-- END spec-trio PM policy -->"
    ...
    if (old.count(start), old.count(stop)) not in ((0, 0), (1, 1)):
        raise ValueError("incomplete or duplicate spec-trio policy markers; file left unchanged")
    block = start + b"\n" + source.read_bytes().rstrip(b"\r\n") + b"\n" + stop
    if start in old:
        left, right = old.index(start), old.index(stop)
        if right < left:
            raise ValueError("reversed spec-trio policy markers; file left unchanged")
    ...
    fd, temporary = tempfile.mkstemp(prefix=".spec-trio-policy-", dir=workspace)
    try:
...
def main() -> None:
    ...
    try:
        ...
        parser.exit(2, f"spec-trio: {exc}\n")
    print(f"Installed spec-trio PM policy: {target}")
