...
- **`dev-trio` plugin** (provides `ask-codex.sh` reviewer + `ask-gemini.sh` researcher on PATH)
...
3. **Stage 2 — Coder** (`claude -p` with `lib/roles/worker.md` + `<spec>` + plan).
4. **Scope gate 2** — every uncommitted/committed/untracked path that the coder produced is compared against the allowlist. If any path is outside, the iteration short-circuits to `OUT-OF-SCOPE` (no reviewer).
5. **Stage 3 — Reviewer** (`ask-codex.sh --with-spec spec.md` against HEAD diff) → verdict: `SHIP / NEEDS-FIX / DISCUSS / OUT-OF-SCOPE`.
6. If codex emits `## NEED RESEARCH`: invoke `ask-gemini.sh`, re-run Stage 2 with research context, re-review.
...
- `--no-research` — skip the NEED RESEARCH branch (no `ask-gemini.sh` dep).
