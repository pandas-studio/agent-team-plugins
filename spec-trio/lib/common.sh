...
#   cp ../ralph-trio/lib/common.sh ./lib/common.sh
# then restore this provenance header. The ralph-trio copy is itself the
# canonical post-port version of core/ralph/lib/common.sh; spec-trio carries
# its own snapshot so it has no cross-plugin sourcing dependency. Drift is
# acceptable: the workspace/team/promise/worktree helpers are a stable
# interface, and tests/smoke-pr5.sh catches breakage on re-sync.
#
...
# spec_workspace_root override at the bottom of this file). The ralph-trio
# variants that share this lib keep using $PWD/.ralph-trio. Both honor an
# explicit RALPH_TRIO_WORKSPACE / SPEC_TRIO_WORKSPACE env var if set.
