...
Installed by `/spec-trio:install-pm`. spec-trio runs an **autonomous, spec-anchored Ralph loop**: a bash driver re-invokes `claude -p` (plus a Codex reviewer and, on NEED RESEARCH, an Antigravity researcher) per iteration until pending tasks are verified and completed, a cap is hit, or human attention is required. Every stage is gated by an **external `spec.md`** — the planner, coder, and reviewer all read it as the contract, and an allowlist + §-citation coverage check keeps the work in scope. You set the loop up and read its results — the loop runs **headless**, not turn-by-turn in this session.
...
spec-trio adds these checks over a bare Ralph loop:
...
- **Required tests** — pass `--test-cmd` for real runs; coder success, scope checks and passing tests precede review. `--autoship` only skips review.
...
3. **Always cap and dry-run first:**
   ...
   spec-trio.sh --spec spec.md --backlog BACKLOG.md --max-iter 20 --max-runtime 4h --coverage-check --test-cmd 'pytest -q'
   ```
4. For risky/parallel work add `--worktree` (each iter runs in a throwaway worktree, fast-forward-merged only on passing tests + SHIP + pre-merge validation; uncommitted coder edits are auto-committed at SHIP so they survive the merge).

## Reporting back to the user

- Report the final status, completed/pending counts, stop reason and exit code. Exit 3 means pending work; exit 4 requires human attention; exit 1 is an execution error.
...
- With `--coverage-check`, report the **§-coverage table** (COVERED / NOT-COVERED per `§5.N`) — this reports implementation traces, not test proof; report recorded test outcomes separately.
...
- Don't edit `spec.md` mid-run. The spec is hashed into each iteration's manifest as the contract; changing or deleting it is detected and stops the run. Stop the loop, amend, restart.
- Don't hand-edit `BACKLOG.md`'s `[x]` markers mid-run; the driver only marks a task done after success and any required merge. NEEDS-FIX retries the original unchecked row; coverage requeue can append new work. Blocking verdicts stop immediately.
